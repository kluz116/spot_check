...
class TellersUsd(models.Model):
    ...
    def _checkbranchspotcheck(self):
        pending_conf = self.env['spot_check.teller_usd'].search([('state', 'in', ['ongoing'])])
    ...
    def _checkbranchspotcheckToDay(self):
        pending_conf = self.env['spot_check.teller_usd'].search([('state', 'in', ['ongoing','confirmed_one','reject_one'])])
        for res in pending_conf:
    ...
    @api.model
    def _accountants_without_spotchecks_usd(self):
        ...
        initiated_req = self.env['spot_check.teller_usd'].search([('state', 'in', ['ongoing','confirmed_one','reject_one'])])
        ...
        for res in obj:
            template_id = self.env.ref('spot_check.email_template_accountants_teller_remiders_usd').id
            template =  self.env['mail.template'].browse(template_id)
            template.send_mail(res.id,force_send=True)

    @api.model
    def _notfy_about_teller_usd(self):
        ...
        for res in obj:
            template_id = self.env.ref('spot_check.email_template_accountants_teller_notfy_usd').id
            template =  self.env['mail.template'].browse(template_id)
            template.send_mail(res.id,force_send=True)

    @api.model
    def _notfy_onetime_teller_usd(self):
        my_date = datetime.date.today()
        week_num = my_date.isocalendar()
        teller_obj = self.env['res.partner'].search([('user_role','in',['teller'])])
        ...
        for result in obj:
            template_id = self.env.ref('spot_check.email_template_accountants_teller_notfy_penind_usd').id
            template =  self.env['mail.template'].browse(template_id)
