...
class ATM(models.Model):
    ...
    @api.model
    def _creditsupervisor_without_spotchecks_atm(self):
        ...
        initiated_req = self.env['spot_check.atm'].search([('state', 'in', ['ongoing','confirmed_one','reject_one'])])
        ...
        for res in obj:
            template_id = self.env.ref('spot_check.email_template_credit_supervisor_remiders_atm').id
            template =  self.env['mail.template'].browse(template_id)
            template.send_mail(res.id,force_send=True)


    @api.model
    def _vault_spot_checked_once_atm(self):
        my_date = datetime.date.today() 
        week_num = my_date.isocalendar()
        branch_obj = self.env['spot_check.branch'].search([('status','in',['active'])])
        branch_list = []
        ...
        for res in obj:
            template_id = self.env.ref('spot_check.email_template_credit_supervisor_once_atm').id
            template =  self.env['mail.template'].browse(template_id)
            template.send_mail(res.id,force_send=True)  